...
final class DashboardWindowController: NSWindowController {
    ...
    private let scaleControl = NSSegmentedControl(labels: ["Fill", "Fit", "Stretch", "Center"], trackingMode: .selectOne, target: nil, action: nil)
    ...
    private func buildUI() {
        ...
        NSLayoutConstraint.activate([
            background.leadingAnchor.constraint(equalTo: content.leadingAnchor), background.trailingAnchor.constraint(equalTo: content.trailingAnchor),
            background.topAnchor.constraint(equalTo: content.topAnchor), background.bottomAnchor.constraint(equalTo: content.bottomAnchor)
        ])
        ...
        let play = actionButton("Apply to Desktop", symbol: "play.fill", action: #selector(applyWallpaper), prominent: true)
        let lockScreen = actionButton("Apply to Lock Screen", symbol: "lock.fill", action: #selector(applyToLockScreen), prominent: true)
        let stop = actionButton("Stop", symbol: "stop.fill", action: #selector(stopWallpaper))
        let pluginBtn = actionButton("Remove Extension", symbol: "trash", action: #selector(toggleExtension))

        chooseButton = choose
        playButton = play
        lockScreenButton = lockScreen
        stopButton = stop
        pluginButton = pluginBtn

        let videoButtons = NSStackView(views: [choose, play, lockScreen, stop, pluginBtn])
        ...
        let displayCard = card(title: "Appearance", subtitle: "Choose how the video fits the display", content: NSStackView(views: [scaleControl, pathLabel]))
        (displayCard.subviews.last as? NSStackView)?.spacing = 12

        loginSwitch.target = self
        loginSwitch.action = #selector(loginChanged)
        let loginRow = NSStackView(views: [NSTextField(labelWithString: "Launch automatically at login"), NSView(), loginSwitch])
        loginRow.orientation = .horizontal
        loginRow.alignment = .centerY
        let startupCard = card(title: "Startup", subtitle: "Keep Livecore ready in the menu bar", content: loginRow)

        statusLabel.textColor = .secondaryLabelColor
        statusLabel.lineBreakMode = .byTruncatingTail
        let stack = NSStackView(views: [preview, videoButtons, displayCard, startupCard, statusLabel])
    ...
    func updateButtonStates() {
        ...
        let isLockScreenActive = AppSettings.shared.lockScreenEnabled
        let isAnyActive = isDesktopActive || isLockScreenActive

        // Buttons enabled when NOT active; Stop button enabled ONLY when active
        stopButton?.isEnabled = isAnyActive
        playButton?.isEnabled = hasVideo && !isDesktopActive
        lockScreenButton?.isEnabled = hasVideo && !isLockScreenActive
    ...
    private func card(title: String, subtitle: String, content: NSView) -> NSView {
        ...
        NSLayoutConstraint.activate([
            stack.leadingAnchor.constraint(equalTo: visual.leadingAnchor, constant: 16), stack.trailingAnchor.constraint(equalTo: visual.trailingAnchor, constant: -16),
            stack.topAnchor.constraint(equalTo: visual.topAnchor, constant: 12), stack.bottomAnchor.constraint(equalTo: visual.bottomAnchor, constant: -12)
        ])
        return visual
    }

    private func actionButton(_ title: String, symbol: String, action: Selector, prominent: Bool = false) -> NSButton {
        let button = NSButton(title: title, target: self, action: action)
        button.image = NSImage(systemSymbolName: symbol, accessibilityDescription: nil)
        button.imagePosition = .imageLeading
        button.bezelStyle = .rounded
        if prominent { button.keyEquivalent = title == "Apply to Desktop" ? "\r" : "" }
        return button
    }

    private func loadSettings() {
        scaleControl.selectedSegment = VideoScaleType.allCases.firstIndex(of: settings.scaleType) ?? 0
        if #available(macOS 13, *) { loginSwitch.state = LoginItemManager.shared.isEnabled ? .on : .off }
        do {
            selectedURL = try settings.resolveVideoURL()
            if let selectedURL { showPreview(selectedURL) }
    ...
    @objc private func applyWallpaper() {
        guard let selectedURL else { setStatus("Choose a video first.", error: true); return }
        do {
    ...
    @objc private func stopWallpaper() {
        guard WallpaperEngine.shared.isActive || AppSettings.shared.lockScreenEnabled else { return }

        WallpaperEngine.shared.stop()
        WallpaperStoreManager.shared.restoreLockScreen()
        AppSettings.shared.lockScreenEnabled = false

        setStatus("Playback stopped. Desktop and Lock Screen restored.")
        updateButtonStates()
    }

    @objc private func scaleChanged() { settings.scaleType = VideoScaleType.allCases[scaleControl.selectedSegment] }

    @objc private func loginChanged() {
        guard #available(macOS 13, *) else { return }
        do { try LoginItemManager.shared.setEnabled(loginSwitch.state == .on) }
        catch { loginSwitch.state = LoginItemManager.shared.isEnabled ? .on : .off; setStatus(error.localizedDescription, error: true) }
    }

    @objc private func applyToLockScreen() {
        guard let selectedURL else { setStatus("Choose a video first.", error: true); return }

        if !WallpaperStoreManager.shared.isPluginInstalled() {
            setStatus("Installing Lock Screen extension…")
            WallpaperStoreManager.shared.installPlugin()
        }

        setStatus("Applying video to Lock Screen…")
        DispatchQueue.global(qos: .userInitiated).async { [selectedURL] in
            do {
                ...
                DispatchQueue.main.async { [weak self] in
                    ...
                    self?.setStatus("Video applied to Lock Screen.")
                }
            } catch {
                DispatchQueue.main.async { [weak self] in
